...
class FreshRSS_EntryDAO extends Minz_ModelPdo implements FreshRSS_Searchable {
	...
	protected function sqlListEntriesWhere($alias = '', $filter = null, $state = FeedReader_Entry::STATE_ALL, $order = 'DESC', $firstId = '', $date_min = 0) {
		$search = ' ';
		$values = array();
		if ($state & FeedReader_Entry::STATE_NOT_READ) {
			if (!($state & FeedReader_Entry::STATE_READ)) {
				$search .= 'AND ' . $alias . 'is_read=0 ';
			}
		}
		elseif ($state & FeedReader_Entry::STATE_READ) {
			$search .= 'AND ' . $alias . 'is_read=1 ';
		}
		if ($state & FeedReader_Entry::STATE_FAVORITE) {
			if (!($state & FeedReader_Entry::STATE_NOT_FAVORITE)) {
				$search .= 'AND ' . $alias . 'is_favorite=1 ';
			}
		}
		elseif ($state & FeedReader_Entry::STATE_NOT_FAVORITE) {
	...
	private function sqlListWhere($type = 'a', $id = '', $state = FeedReader_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filter = '', $date_min = 0) {
		if (!$state) {
			$state = FeedReader_Entry::STATE_ALL;
		}
		$where = '';
		$joinFeed = false;
		$values = array();
	...
	public function listWhereRaw($type = 'a', $id = '', $state = FeedReader_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filter = '', $date_min = 0) {
	...
	public function listWhere($type = 'a', $id = '', $state = FeedReader_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filter = '', $date_min = 0) {
	...
	public function listIdsWhere($type = 'a', $id = '', $state = FeedReader_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filter = '', $date_min = 0) {	//For API
		list($values, $sql) = $this->sqlListWhere($type, $id, $state, $order, $limit, $firstId, $filter, $date_min);
