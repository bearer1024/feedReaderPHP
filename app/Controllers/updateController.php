...
class FreshRSS_update_Controller extends Minz_ActionController {
	...
	public static function isGit() {
		return is_dir(FEEDREADER_PATH . '/.git/');
	}

	public static function hasGitUpdate() {
		$cwd = getcwd();
		chdir(FEEDREADER_PATH);
		$output = array();
		try {
	...
	public function indexAction() {
		...
		if (!is_writable(FEEDREADER_PATH)) {
			$this->view->message = array(
				'status' => 'bad',
				'title' => _t('gen.short.damn'),
				'body' => _t('feedback.update.file_is_nok', FEEDREADER_PATH)
			);
	...
	public function applyAction() {
		if (!file_exists(UPDATE_FILENAME) || !is_writable(FEEDREADER_PATH) || Minz_Configuration::get('system')->disable_update) {
			Minz_Request::forward(array('c' => 'update'), true);
		}
